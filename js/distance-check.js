...
var turret = createTurret(new Point(300, 250), 175);
var player = createPlayer(new Point(view.center), 25);

function onMouseMove(event) {
  player.position = event.point;

  var distanceSquared = ((turret.position.x - player.position.x) * (turret.position.x - player.position.x)) + ((turret.position.y - player.position.y) * (turret.position.y - player.position.y));


  var radiusSquared = turret.meta.getRadius() * turret.meta.getRadius();
