...
`),b.markdown=o[0]&&typeof o[0].getAttribute("data-markdown")=="string"),n.postMessage(JSON.stringify(b),"*")}function u(d){try{return window.location.origin===d.source.location.origin}catch{return!1}}function m(d){if(u(d))try{let p=JSON.parse(d.data);p&&p.namespace==="reveal-notes"&&p.type==="connected"?(clearInterval(l),f()):p&&p.namespace==="reveal-notes"&&p.type==="call"&&a(p.methodName,p.arguments,p.callId)}catch{}}function f(){e.on("slidechanged",r),e.on("fragmentshown",r),e.on("fragmenthidden",r),e.on("overviewhidden",r),e.on("overviewshown",r),e.on("paused",r),e.on("resumed",r),e.on("previewiframe",r),e.on("previewimage",r),e.on("previewvideo",r),e.on("closeoverlay",r),r()}return{id:"notes",init:function(d){e=d,/receiver/i.test(window.location.search)||(window.location.search.match(/(\?|\&)notes/gi)!==null?t():window.addEventListener("message",p=>{if(!n&&typeof p.data=="string"){let o;try{o=JSON.parse(p.data)}catch{}o&&o.namespace==="reveal-notes"&&o.type==="heartbeat"&&i(p.source)}}),e.addKeyBinding({keyCode:83,key:"S",description:"Speaker notes view"},function(){t()}))},open:t}}});
