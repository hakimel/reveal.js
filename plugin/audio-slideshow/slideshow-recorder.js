...
var Recorder = {
	...
	// Create canvas on which red circle over auto slide control can be drawn
	this.canvas = document.createElement( 'canvas' );
	this.canvas.className = 'playback';
	this.canvas.width = 50;
	this.canvas.height = 50;
	this.canvas.addEventListener( 'click', function( event ) { Reveal.toggleAutoSlide(); }, true );
...
//	document.title = this.isRecording ? "Recorder: On" : "Recorder: Off";
    },
	...
	this.indices = Reveal.getIndices();
		// add audio element to slide if necessary
		var container = Reveal.getCurrentSlide();
		var currentFragment = container.querySelector( '.current-fragment' );
		if ( currentFragment ) {
			container = currentFragment;
		}
		this.recordedAudio = container.querySelector( '.recorded' );
		if ( !this.recordedAudio ) {
			this.recordedAudio = document.createElement( 'audio' );
			this.recordedAudio.setAttribute( 'class', 'recorded' );
			container.insertBefore( this.recordedAudio, container.firstChild );
		}		
...
//------------------------------------
// Browsers Support::
// Chrome (all versions) [ audio individually ]
...
function RecordRTC(mediaStream, config) {
    ...
    function startRecording() {
        console.debug('started recording ' + (IsChrome ? config.type : 'audio') + ' stream.');
    ...
    function stopRecording(callback) {
        ...
        console.warn('stopped recording ' + (IsChrome ? config.type : 'audio') + ' stream.');
